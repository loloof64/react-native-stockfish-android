...
public abstract class UCIEngineBase implements UCIEngine {
    ...
    public static UCIEngine getEngine(Context context, String engine,
        ...
        if ("stockfish".equals(engine))
            return new InternalStockFish(context, report, engineOptions.workDir);
        else if (EngineUtil.isOpenExchangeEngine(engine))
            return new OpenExchangeEngine(context, engine, engineOptions.workDir, report);
        else
            return new ExternalEngine(context, engine, engineOptions.workDir, report);
    }
